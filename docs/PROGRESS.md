...
| 2026-07-03 | P5++ | ◐ gate run | `test(enh): enhancement re-calibration gate` (branch `enhancement-gate`) | 197 passing | **OFS / valuation / anchor gated (GMP-parity).** Backfill **293/293** Chittorgarh research pull, name-verified (0 false joins). **All fail:** OFS **CUT**, valuation **NOT EARNED**, anchor **NOT TESTED** (list API-rendered) — QIB-redundant like GMP. **Finding 1:** OFS kill-flag is **backwards** (near-total-OFS 17% loss vs pure-fresh 26%) — do NOT implement `OFS→SKIP`. **Finding 2:** valuation's lift was an **outlier artifact** (evaporated on the hand-QA'd 93) — data-QA before the gate was essential. Dead-feature weights → **0.0** (byte-equality MAX\|Δprob\|=0.0); shipped calibrator untouched. See [ENHANCEMENT_GATE.md](ENHANCEMENT_GATE.md). |
