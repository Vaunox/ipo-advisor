...
> **PROBED (2026-07-05) — the free-data route is CLOSED with evidence; NOT a permanent closure.** The only historical day-wise dataset (arXiv 2412.16174 / HF `sohomghosh/Indian_IPO_datasets`, Chittorgarh) is **endpoint-reliable but path-useless**: its final totals match our NSE finals (**98%**) but the day-wise columns **truncate at day 2**, missing the close-day QIB surge — so it reaches our verified final on only **0.8%** of the 2017–2023 overlap (e.g. Indigo Paints: our QIB 189.6× vs their last day 5.4×). A salvage probe (their day-2 + our verified final → final-day surge share, **N=120**, full gate harness) showed **no pulse** (lift ≈0 all splits, CI straddling zero, ECE worse), consistent with QIB-redundancy — but on probe-grade-squared data this is **not** a permanent closure. **B1's honest state: neither rejected nor open — unreachable without collection, and the best free look found nothing.** A real B1 gate **requires forward collection (recorder)**; no cheap positive signal exists to chase. Evidence: `docs/B1_PROBE.md`, `research/run_b1_probe.py`.
